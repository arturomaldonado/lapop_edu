...
## Introducción
En este documento se explicará cómo introducir el factor de expansión y las ponderaciones en los cálculos estadísticos usando los datos del Barómetro de las Américas. Seguiremos lo que se trabajó en el documento de "Manejo de datos", disponible [aquí](https://rpubs.com/arturo_maldonado/727887). 
...
Para reproducir los datos que se observan en el Gráfico 1.2 del reporte, se puede usar el comando `compmeans` que permite calcular la media de una variable por grupos de una variable factor, como "pais", ponderando los resultados por una variable, como "weight1500". Se agrega la especificación `plot=FALSE` para desactivar la producción del gráfico.
