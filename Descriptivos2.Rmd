...
De la misma manera que con las variables nominales, estas variables tienen que ser declaradas como "factor" en una nueva variable.
```{r factor}
...
Para producir las tablas con porcentajes.
```{r porcentajes}
...
df
library(ggplot2)
ggplot(df, aes(x="", y=per, fill=smedia2r))+
...
En la página 55 del reporte "El pulso de la democracia" se presenta los porcentajes de uso de las redes sociales por país. Para replicar esta tabla primero se tiene que definir la variable `pais` y las variables de uso de redes sociales.
...
Empezaremos replicando el cuadro de uso de redes sociales por país.
```{r país}
...
En primer lugar, se crean tablas bivariadas entre cada red social y país. Estas tablas presentan el % de quienes usan y el % de los que no usan cada red social.
